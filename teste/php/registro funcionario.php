...
if(!empty($_POST)){
	if(isset($_POST["username"]) &&isset($_POST["fullname"]) &&isset($_POST["email"]) &&isset($_POST["password"]) &&isset($_POST["confirm_password"])){
		if($_POST["username"]!=""&& $_POST["fullname"]!=""&&$_POST["email"]!=""&&$_POST["password"]!=""&&$_POST["password"]==$_POST["confirm_password"]){
			...
			$fone = $_POST['fone'];
			$cpf = $_POST['cpf'];
			$sql = "insert into user(username,fullname,email,password,created_at,funcionario,rua,cep,numero,fone,cpf) value (\"$_POST[username]\",\"$_POST[fullname]\",\"$_POST[email]\",\"$_POST[password]\",NOW(), '1',\"$_POST[rua]\",\"$_POST[cep]\",\"$_POST[numero]\",\"$_POST[fone]\",\"$_POST[cpf]\")";
			$query = $con->query($sql);
			if($query!=null){
